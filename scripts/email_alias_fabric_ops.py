...
DEFAULT_STATE_ROOT = Path(os.environ.get("CADUCEUSMAIL_STATE_DIR", "")).expanduser() if os.environ.get("CADUCEUSMAIL_STATE_DIR") else (Path.home() / ".caduceusmail")

if os.environ.get("CADUCEUSMAIL_INTEL_DIR"):
    INTEL = Path(os.environ["CADUCEUSMAIL_INTEL_DIR"]).expanduser()
else:
...
def credentials_file_candidates() -> list[Path]:
    out: list[Path] = []
    if os.environ.get("CADUCEUSMAIL_CREDENTIALS_DIR", "").strip():
        out.append(Path(os.environ["CADUCEUSMAIL_CREDENTIALS_DIR"]).expanduser())
